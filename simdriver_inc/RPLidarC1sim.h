#ifndef RPLIDARC1SIM_H
#define RPLIDARC1SIM_H
...
class RPLidarC1 {
  public:
    ...
    int
    get_measurements(RPLidarMeasurement* measurements, int max_count, uint32_t timeout_ms = 1000);
  ...
  private:
    ...
    // Tracks which point in lidarData to hand out next when running in sim.
    // Lives here so it persists across calls to get_measurement().
    int _simIndex;
